#let slides(
	...
	let hasBackground(selector, loc) = query(selector)
	...
	set page(
			...
			#let sectionsWithPages = sections.enumerate(start: 1).map(((i, s)) => {(
				...
				grid(
					...
					..sectionsWithPages.map(((pageRange, label)) => [
						#let is-active = pageRange.contains(here().page()-1)
		...
		footer-descent: 2em,
		footer: context [
			#if hasBackground(selector(<background>).before(here()), here()) == false {
				set text(
					size: 0.4em,
	...
	show link: l => {
		if type(l.dest) == str and l.dest.starts-with("http") {
			underline(l)
		} else {
			l
		}
